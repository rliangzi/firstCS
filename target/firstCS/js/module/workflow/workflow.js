function graphTrace(options) {
    ...
    $.getJSON(ctx + '/workflow/process/trace?pid=' + opts.pid + "&peid=" + opts.peid, function(infos) {
        var positionHtml = "";
        // 生成图片
        ...
        if ($('#workflowTraceDialog').length == 0) {
            $('<div/>', {
                ...
                title: '查看流程（按ESC键可以关闭）<button id="diagram-viewer">Diagram-Viewer</button>',
        ...
        $('#workflowTraceDialog').dialog({
            ...
            dragable: false,
            open: function() {
                $('#workflowTraceDialog').dialog('option', 'title', '查看流程（按ESC键可以关闭）<button id="diagram-viewer">Diagram-Viewer</button>');
